...
addLayer("g", {
    ...
    effectDescription() {
        try { 
            ...
            // ASYMMETRIC CROSS-INFLATION SYSTEM: Starts strictly at 1,000,000 points (1M Wall)
    cost(x) {
        try {
            ...
            if (player.b && player.b.points) {
                let b_amt = new Decimal(player.b.points);
                if (b_amt.gt(0)) {
                    formula = formula.times(Decimal.pow(25, b_amt.pow(1.1)));
                }
            }
